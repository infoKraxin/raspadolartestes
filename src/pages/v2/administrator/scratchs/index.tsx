...
const mockScratchCards: ScratchCard[] = [
  {
    id: "mock-scratch-1",
    name: "Raspadinha da Fortuna (Mock)",
  ...
  },
  {
    ...
    prizes: [
      {
        id: "p2",
        scratchCardId: "mock-scratch-2",
        name: "iPhone",
        description: "",
        type: "PRODUCT",
        value: "0",
        product_name: "iPhone 15",
        redemption_value: "7000",
        image_url: null,
        probability: "0.05",
        is_active: true,
        created_at: "",
  ...
  },
  {
    ...
    prizes: [
      {
        id: "p3",
        scratchCardId: "mock-scratch-3",
        name: "Moto",
        description: "",
        type: "PRODUCT",
        value: "0",
        product_name: "Moto 0km",
        redemption_value: "12000",
        image_url: null,
        probability: "0.01",
        is_active: true,
        created_at: "",
...
export default function ScratchCardsPage() {
  ...
  const { token, isLoading: authLoading } = useAuth() // token e authLoading ainda são usados para simular o estado de autenticação
  ...
  // Simula o carregamento das raspadinhas
  ...
  useEffect(() => {
    // Carrega as raspadinhas mockadas assim que o componente é montado
    fetchScratchCards()
  }, [])
  ...
  const confirmDelete = async () => {
    ...
    setTimeout(() => {
      if (Math.random() > 0.1) {
        ...
        toast.success("Raspadinha excluída com sucesso (simulado)!")
      } else {
        toast.error("Erro simulado ao excluir raspadinha.")
      }
      setDeleting(false)
    }, 1000)
  }
  ...
  const confirmToggleFeatured = async () => {
    ...
    setFeaturing(true)
    // Simula a alteração de destaque
    setTimeout(() => {
      if (Math.random() > 0.1) {
        // 90% de chance de sucesso simulado
        setScratchCards((prev) =>
          prev.map((card) => (card.id === cardToFeature.id ? { ...card, is_featured: !card.is_featured } : card)),
        )
        setFeatureModalOpen(false)
        setCardToFeature(null)
        toast.success(`Destaque ${cardToFeature.is_featured ? "removido" : "adicionado"} (simulado)!`)
      } else {
        toast.error("Erro simulado ao alterar destaque da raspadinha.")
      }
      setFeaturing(false)
    }, 1000)
  }
  ...
  const totalRevenue = scratchCards.reduce((sum, card) => sum + Number.parseFloat(card.total_revenue || "0"), 0)
  ...
  const getMaxPrize = (prizes: Prize[]) => {
    ...
    return Math.max(...prizes.map((prize) => Number.parseFloat(prize.value || "0")))
  }
            ...
            <Card className="bg-neutral-800 border-neutral-700">
              <div className="p-6 border-b border-neutral-700">
                ...
                <p className="text-neutral-400 text-sm">
                  Mostrando {filteredCards.length} de {scratchCards.length} raspadinhas
                </p>
              </div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {filteredCards.map((card) => (
                        ...
                        >
                          <td className="p-4">
                            <span className="text-neutral-300 text-sm font-mono">{card.id.substring(0, 8)}...</span>
                          </td>
                          <td className="p-4">
                          ...
                          <td className="p-4">
                            <span className="text-neutral-300 text-sm max-w-xs truncate block">{card.description}</span>
                          </td>
                          <td className="p-4">
                          ...
                          <td className="p-4">
                            <span className="text-neutral-300">{Number.parseFloat(card.target_rtp).toFixed(1)}%</span>
                          </td>
                          <td className="p-4">
                            <span
                              className={`font-medium ${
                                Number.parseFloat(card.current_rtp) >= Number.parseFloat(card.target_rtp)
                                  ? "text-green-400"
                                  : "text-yellow-400"
                              }`}
                            >
                              {Number.parseFloat(card.current_rtp).toFixed(1)}%
                            </span>
                          </td>
                          <td className="p-4">
                            <span className="text-neutral-300">{card.total_games_played.toLocaleString("pt-BR")}</span>
                          </td>
                          <td className="p-4">
                            <span className="text-white font-medium">{formatCurrency(card.total_revenue)}</span>
                          </td>
                          <td className="p-4">
                          ...
                          <td className="p-4">
                            <span className="text-neutral-300 text-sm">{formatDate(card.created_at)}</span>
                          </td>
                          <td className="p-4">
                            <div className="flex items-center gap-2">
                              <Button
        ...
        <DialogContent className="bg-neutral-800 border-neutral-700 text-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-neutral-400">
              ...
              ?
              <br />
              Esta ação não pode ser desfeita. (Simulado)
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-neutral-400">
              {cardToFeature?.is_featured ? (
                <>
                  Tem certeza que deseja remover o destaque da raspadinha{" "}
                  <strong className="text-white">"{cardToFeature?.name}"</strong>?
                  <br />
                  Ela não aparecerá mais em destaque na página inicial. (Simulado)
                </>
              ) : (
                <>
                  Tem certeza que deseja destacar a raspadinha{" "}
                  <strong className="text-white">"{cardToFeature?.name}"</strong>?
                  <br />
                  Ela aparecerá em destaque na página inicial. (Simulado)
                </>
              )}
            </DialogDescription>
          </DialogHeader>
