...
export default function CreateScratchCard() {
  ...
  const { token } = useAuth() // token ainda é usado para simular o estado de autenticação
  ...
  const removePrize = (index: number) => {
    if (prizes.length > 1) {
      ...
      toast.error("É necessário ter pelo menos um prêmio.")
    }
  }
  ...
  const distributeProbabilities = () => {
    if (prizes.length === 0) {
      toast.error("Adicione pelo menos um prêmio para distribuir probabilidades.")
      return
    }

    // O primeiro prêmio (mais importante) recebe uma probabilidade menor
    const firstPrizeProbability = 1.0 // Exemplo: 1% para o primeiro prêmio

    // Distribuir o restante das probabilidades entre os outros prêmios
    const remainingProbability = 99.0 // 99% para os demais
    const probabilityPerPrize = remainingProbability / (prizes.length > 1 ? prizes.length - 1 : 1)
    ...
    setPrizes(updatedPrizes)
    toast.success("Probabilidades distribuídas (simulado)!")
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validações básicas
    if (!scratchCard.name || !scratchCard.description || !scratchCard.image_url) {
      toast.error("Por favor, preencha todos os campos obrigatórios da raspadinha.")
      return
    }

    if (scratchCard.price <= 0) {
      toast.error("O preço da raspadinha deve ser maior que zero.")
      return
    }

    if (scratchCard.target_rtp <= 0 || scratchCard.target_rtp > 100) {
      toast.error("O RTP Alvo deve ser entre 0 e 100.")
      return
    }

    const prizesWithoutImage = prizes.filter((prize) => !prize.image_url)
    if (prizesWithoutImage.length > 0) {
      toast.error("Por favor, adicione URLs de imagem para todos os prêmios.")
      return
    }

    const invalidPrizes = prizes.filter((prize) => {
      if (!prize.name || !prize.description || prize.probability <= 0 || prize.probability > 100) return true
      if (prize.type === "MONEY" && (prize.value === undefined || prize.value <= 0)) return true
      if (
        prize.type === "PRODUCT" &&
        (!prize.product_name || prize.redemption_value === undefined || prize.redemption_value <= 0)
      )
        return true
      return false
    })
    ...
    if (invalidPrizes.length > 0) {
      toast.error("Por favor, preencha todos os campos obrigatórios e válidos para cada prêmio.")
      return
    }

    const totalProbability = prizes.reduce((sum, prize) => sum + prize.probability, 0)
    if (totalProbability > 100.01 || totalProbability < 99.99) {
      // Pequena margem de erro para floats
      toast.error(`A soma das probabilidades dos prêmios deve ser 100%. Atual: ${totalProbability.toFixed(2)}%`)
      return
    }

    setLoading(true)

    // Simula a criação da raspadinha
    setTimeout(() => {
      if (Math.random() > 0.1) {
        // 90% de chance de sucesso simulado
        toast.success("Raspadinha criada com sucesso (simulado)!")
        router.push("/v2/administrator/scratchs")
      } else {
        toast.error("Erro simulado ao criar raspadinha.")
      }
      setLoading(false)
    }, 1500)
  }
              ...
              <Card className="bg-neutral-800 border-neutral-700">
                <CardHeader>
                  ...
                  <CardDescription className="text-neutral-400">
                    Configure os dados básicos da raspadinha
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setScratchCard({ ...scratchCard, price: Number.parseFloat(e.target.value) || 0 })
                        }
                        placeholder="10.00"
                        className="bg-neutral-700 border-neutral-600 text-white placeholder:text-neutral-400 focus:border-yellow-500"
                        required
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="target_rtp" className="text-neutral-300">
                        RTP Alvo (%) *
                      </Label>
                      <Input
                        id="target_rtp"
                        ...
                        onChange={(e) =>
                          setScratchCard({ ...scratchCard, target_rtp: Number.parseFloat(e.target.value) || 0 })
                        }
                        placeholder="85.0"
                        className="bg-neutral-700 border-neutral-600 text-white placeholder:text-neutral-400 focus:border-yellow-500"
                        required
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardDescription className="text-neutral-400">
                        Configure os prêmios disponíveis na raspadinha
                      </CardDescription>
                    </div>
                    ...
                    <div key={index} className="border border-neutral-600 rounded-lg p-4 space-y-4 bg-neutral-700/50">
                        ...
                        <div className="space-y-2">
                          ...
                          >
                            ...
                            <SelectContent className="bg-neutral-700 border-neutral-600">
                              <SelectItem value="MONEY" className="text-white hover:bg-neutral-600">
                                Dinheiro
                              </SelectItem>
                              <SelectItem value="PRODUCT" className="text-white hover:bg-neutral-600">
                                Produto
                              </SelectItem>
                            </SelectContent>
                          </Select>
                      ...
                      {prize.type === "MONEY" ? (
                        <div className="space-y-2">
                          ...
                          <Input
                            ...
                            min="0"
                            value={prize.value || ""}
                            onChange={(e) => updatePrize(index, "value", Number.parseFloat(e.target.value) || 0)}
                            placeholder="100.00"
                            className="bg-neutral-700 border-neutral-600 text-white placeholder:text-neutral-400 focus:border-yellow-500"
                            required
                          />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                updatePrize(index, "redemption_value", Number.parseFloat(e.target.value) || 0)
                              }
                              placeholder="500.00"
                              className="bg-neutral-700 border-neutral-600 text-white placeholder:text-neutral-400 focus:border-yellow-500"
                              required
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => updatePrize(index, "probability", Number.parseFloat(e.target.value) || 0)}
                          placeholder="5.0"
                          className="bg-neutral-700 border-neutral-600 text-white placeholder:text-neutral-400 focus:border-yellow-500"
                          required
                        />
