...
const formatCurrency = (amount: string, symbol = "R$") => {
...
export default function AdminDepositsPage() {
  ...
  const stats = {
    totalApproved: deposits.filter((d) => d.status === true).reduce((sum, d) => sum + Number.parseFloat(d.amount), 0),
    totalPending: deposits.filter((d) => d.status === false).reduce((sum, d) => sum + Number.parseFloat(d.amount), 0),
